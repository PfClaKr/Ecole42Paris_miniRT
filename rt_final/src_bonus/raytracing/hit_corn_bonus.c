...
/*   Updated: 2023/07/31 21:24:40 by ychun            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	get_corn_normal(t_cylinder *obj, t_hit_record *rec, double hit_height, double r, t_vec3 p)
{
	double	max_height;
	t_vec3	hit_normal;
	t_vec3	hit_pos;
	...
	if (hit_height >= max_height || hit_height < 2)
		return (0);
...
double	get_corn_hit_height(t_cylinder *obj, t_ray ray)
{
	t_hit_num	num;
	double		ret;
	...
	ret = vdot(vminus(vplus(ray.pos, vmult(ray.orivec, num.root)),
				obj->pos), obj->orivec);
	return (ret);
}
...
int	hit_corn_side(t_cylinder *obj, t_ray ray, t_hit_record *rec)
{
	...
	num.p = vminus(ray.pos, obj->pos);
	hit_height = get_corn_hit_height(obj, ray);
	if (hit_height == -1)
		return (0);
	r = ((obj->diameter / 2) * (obj->height - hit_height)) / obj->height;
	num.a = vlength2(vcross(ray.orivec, obj->orivec));
	num.half_b = vdot(vcross(ray.orivec, obj->orivec),
			vcross(num.p, obj->orivec));
	...
	if (!get_corn_normal(obj, rec, hit_height, r, vplus(ray.pos, vmult(ray.orivec, num.root))))
		return (0);
